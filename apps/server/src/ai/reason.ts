...
SEMANTIC CONTINUITY (changeType "moved" on a "Section"):
A group whose beforeValue/afterValue are two different section names (e.g. "Key Highlights" → "Key Benefits", "Product Description" → "Product Details") means the diff engine already determined — by comparing the section's actual child content, not just its name — that this is the SAME underlying section that was renamed or reorganized, not one section deleted and an unrelated one created. Describe it that way: the section was renamed/reorganized/repositioned, its underlying information stayed the same. Never say "removed" and "added" for this pair, and never claim the underlying content changed unless a separate group in the same report actually reports a content change for it. This is typically low-to-medium significance — it's a presentation/structure change, not new information.`;
